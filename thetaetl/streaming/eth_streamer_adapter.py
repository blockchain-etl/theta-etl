...
from thetaetl.jobs.export_traces_job import ExportTracesJob
from thetaetl.jobs.extract_contracts_job import ExtractContractsJob
from thetaetl.jobs.extract_token_transfers_job import ExtractTokenTransfersJob
from thetaetl.jobs.extract_tokens_job import ExtractTokensJob
from thetaetl.streaming.enrich import enrich_transactions, enrich_logs, enrich_token_transfers, enrich_traces, \
    enrich_contracts, enrich_tokens
from thetaetl.streaming.eth_item_id_calculator import EthItemIdCalculator
from thetaetl.streaming.eth_item_timestamp_calculator import EthItemTimestampCalculator
...
class EthStreamerAdapter:
    ...
    def _export_traces(self, start_block, end_block):
        ...
        job = ExportTracesJob(
            ...
            end_block=end_block,
            batch_size=self.batch_size,
            web3=ThreadLocalProxy(lambda: self.batch_web3_provider),
            max_workers=self.max_workers,
            item_exporter=exporter
        )
        job.run()
